...
As a note, matrix multiplications are embarrassingly parallel operations; however, FFTs are not.
...
Even so, there are methods to reframe these problems such that they are better optimized for massively parallel devices, and these will be covered when relevant to the development of GPUE.
...
In addition the data transfer between GPU's and between the GPU and CPU through the PCI bandwidth is a slow process.
...
Throughout these sections, we will discuss performance tips to ensure maximum GPU utilization, memory throughput, and intruction throughput.
...
This means that important elements of this programming interface will appear in subsequent sections regarding hardward limitations and performance guidelines.
...
To start, let us assume a simple example where we would like to add two vectors such that $\mathbf{a} + \mathbf{b} = \mathbf{c}$.
...
If we take the above example of vector addition, assuming that the thread count is higher than can fit in one block and take into consideration potential out-of-bounds behaviour, the kernel would instead look like Listing~\ref{lst:vecaddCUDA2}.
...
The tradeoff between performance and readability in programming has been described as the ``two-language'' problem, as most scientific computing solutions to this point have required using two languages: a fast language for the back-end and a readable language for the user interface.
...
\noindent where \texttt{b} is the batch ID, \texttt{x} is the element ID, \texttt{idist} and \texttt{odist} are the distances between batches for the input and output array, respectively, and \texttt{istride} and \texttt{ostride} are the strides between consequtive elements for computation with the input and output array, respectively.
...
\jrs{Not sure if images will help here}
...
Next, we will focus on another feature that was inhibited by the VUDA framework, but is nevertheless possible: methods to enable dynamic quantum engineering with expression trees.
...
In addition, sound waves and other perterbations with minimal density can occur.
...
For this reason, we have begun the development of GPUE.jl, which has similar performance to GPUE, but is currently lacking the expression tree functionality.
...
the current recommended method for tranposes on massively parallel devices requires heavy use of shared memory tiles to speed up the process~\cite{harris2013}.
