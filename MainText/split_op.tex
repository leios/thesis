...
The dynamics of BEC systems can be described by the Gross--Pitaevskii equation, shown in Equation~\ref{eqn:GPintro}, and for this section, we will follow a straightforward derivation with the second quantization~\cite{aversa2008}.
...
Every singly-charged vortex in a BEC will have a $2\pi$ phase winding, and an example simulation with one vortex and its corresponding phase can be seen in FIGURE (a and b).

\begin{figure}

\begin{centering}
\includegraphics[width=0.48\textwidth]{data/splitop/rot/density_L1.pdf}
\includegraphics[width=0.48\textwidth]{data/splitop/rot/phase_L1.pdf}

\includegraphics[width=0.48\textwidth]{data/splitop/rot/density_L10.pdf}
\includegraphics[width=0.48\textwidth]{data/splitop/rot/phase_L10.pdf}
...
An example of this vortex lattice and its phase can be seen in FIGURE (c and d).

\jrs{ADD FIGURE OF SINGLE VORTEX AND VORTEX LATTICE WITH PHASE}
...
As mentioned, rotation of a BEC system will provide vortex lines that follow the axis of rotation and start and end on the BEC boundary, and examples of vortex lines in two-dimensions can be seen in FIGURE (above).
To simulate the effects of rotation, we simply need to append the angular momentum operator $L_z = xp_y - yp_x$ to the GPE:
...
In order to generate a vortex via rotation, we must rotate faster than the critical velocity of $\Omega_c = 0.3 \omega_x$, where $\omega_x$ is the trapping frequency.
In addition, if the rotation frequecy is greater than the trapping frequency, the atoms will no longer be bound by the trap.
...
Largescale vortex lattices have been generated both experimentally and theoretically, with the largest vortex lattices being generated with $\Omega \approx \omega_x$ \cite{o2016, o2016topo, abo2001}.
...
\section{Comparison with other methods for simulating superfluid systems}
\begin{itemize}
\item Point-vortex / vortex filament
\item RK
\end{itemize}
